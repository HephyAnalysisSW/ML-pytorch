...
class Node:
    ...
    def split(self, _depth=0):
        ...
        if  self.max_depth <= _depth+1 or (not any(self.split_left_group)) or all(self.split_left_group): # Jason Brownlee starts counting depth at 1, we start counting at 0, hence the +1
            ...
            self.left        = ResultNode(**{val:func(np.ones(self.size,dtype=bool)) for val, func in result_funcs.items()})
            self.right       = ResultNode(**{val:func(np.zeros(self.size,dtype=bool)) for val, func in result_funcs.items()})
            # The split was good, but we stop splitting further. Put the result of the split in the left/right boxes.
            #self.left, self.right = ResultNode(**{val:func(self.split_left_group) for val, func in result_funcs.items()}), ResultNode(**{val:func(~self.split_left_group) for val, func in result_funcs.items()})
            return
        # process left child
