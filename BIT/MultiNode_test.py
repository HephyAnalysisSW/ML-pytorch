...
argParser.add_argument("--nTraining",          action="store",      default=100,        type=int,  help="number of training events")
argParser.add_argument("--coefficients",       action="store",      default=['theta1'],  nargs="*", help="Maximum number of splits in node split")
...
model.first_derivatives  = list(filter( lambda der: all( [ c in args.coefficients for c in der ] ), model.first_derivatives ))
model.second_derivatives = list(filter( lambda der: all( [ c in args.coefficients for c in der ] ), model.second_derivatives ))
...
base_point_const = np.array([[ functools.reduce(operator.mul, [point[coeff] if coeff in point else 0 for coeff in der ], 1) for der in model.derivatives] for point in base_points]).astype('float')
...
for i_feature in range(len(features[0])):
    ...
    r       = np.dot( sorted_weight_sums, base_point_const.transpose())**2/sorted_weight_sums[:,0].reshape(-1,1)
    r_right = np.dot( sorted_weight_sums_right, base_point_const.transpose())**2/sorted_weight_sums_right[:,0].reshape(-1,1)

    #r       = np.dot( sorted_weight_sums, base_point_const.transpose())/sorted_weight_sums[:,0].reshape(-1,1)
    #r_right = np.dot( sorted_weight_sums_right, base_point_const.transpose())/sorted_weight_sums_right[:,0].reshape(-1,1)
    neg_loss_gains  = np.sum( sorted_weight_sums[:,0].reshape(-1,1)*      ( np.log((1./(1.+r)))         + r      *np.log((r/(1.+r))) ), axis=1)
    neg_loss_gains += np.sum( sorted_weight_sums_right[:,0].reshape(-1,1)*( np.log((1./(1.+r_right)))   + r_right*np.log((r_right/(1.+r_right))) ), axis=1)
