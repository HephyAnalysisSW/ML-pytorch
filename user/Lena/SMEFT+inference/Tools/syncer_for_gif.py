...
def copygif_(name, dir_):
    shutil.copyfile(name, os.path.join(dir_,name))

_savegif = shutil.copyfile

def my_savegif( *args, **kwargs):
    if not os.path.exists(os.path.dirname( args[1] ) ):
        os.makedirs( os.path.dirname( args[1] ) )
    print("hi")    
    file_sync_storage.append( os.path.join(args[1]) )
    _savegif(*args, **kwargs)

shutil.copyfile = my_savegif
