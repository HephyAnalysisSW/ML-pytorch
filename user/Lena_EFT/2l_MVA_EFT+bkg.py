...
argParser.add_argument('--animate' ,           action='store',                   default=None,     choices=["FULL", "sig", "sig+bkg"], help="make an animation of full / sig / separated sig+bkg data?")
...
assert not (args.add_bkg==False and (args.animate == ("FULL" or "sig+bkg"))), "cannot animate signal and bkg if --add_bkg is not set!"
...
def eval_train ( var_evaluation ):
    ...
    if  (args.animate=="sig" or args.animate=="sig+bkg"):
        ...
        if (args.animate=="sig+bkg"):
            ...
            for b in range ( 1,len(bins)-1 ):
                for ind in range ( x_eval_bkg.shape[0] ):
                    val = x_eval_bkg[ind]
                    if ( val > bins[b-1] and val<= bins[b] ):
                        train_lin[b] += z_bkg[ind,0]  
                        train_quad[b]+= z_bkg[ind,1]                                 
    ...
    if (args.animate == "FULL"):
        ...
        for b in range ( 1,len(bins)-1 ):
            for ind in range ( x_eval.shape[0] ):
                val = x_eval[ind]
                if ( val > bins[b-1] and val<= bins[b] ):
                    truth_lin[b] +=W_np[ind,1]   / W_np[ind,0] 
                    truth_quad[b]+=W_np[ind,2]   / W_np[ind,0] 
    ...
    if  (args.animate=="sig" or args.animate=="sig+bkg"):
        ...
        for b in range ( 1,len(bins)-1 ):
            for ind in range ( x_eval.shape[0] ):
                val = x_eval[ind]
                if ( val > bins[b-1] and val<= bins[b] ):
                    truth_lin[b] +=w[ind,1] / w[ind,0] 
                    truth_quad[b]+=w[ind,2] / w[ind,0]            
        i = plotvars.index( var_evaluation )
        name_var = var_evaluation
        plots[name_var+'truelin'],  = ax[index[i]].plot( bins,truth_lin[:,0],  drawstyle='steps', label = "lin truth",   color='orange' ,linestyle = 'dotted' )
        plots[name_var+'truequad'], = ax[index[i]].plot( bins,truth_quad[:,0], drawstyle='steps', label = "quad truth",  color='red'    ,linestyle = 'dotted' )           
        ...
        if (args.animate=="sig+bkg"):
            ...
            for b in range ( 1,len(bins)-1 ):
                for ind in range ( x_eval_bkg.shape[0] ):
                    val = x_eval_bkg[ind]
                    if ( val > bins[b-1] and val<= bins[b] ):
                        truth_lin[b] +=w_bkg[ind,1]  / w_bkg[ind,0] 
                        truth_quad[b]+=w_bkg[ind,2]  / w_bkg[ind,0]         
            name_var = var_evaluation+"_bkg"
