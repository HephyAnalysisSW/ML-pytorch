...
argParser.add_argument('--input_directory',    action='store',      type=str,    default='/eos/vbc/group/cms/lena.wild/tttt/training-ntuples-tttt_v6_1/MVA-training/')
...
argParser.add_argument('--animate' ,           action='store',                   default='sig',    choices=["FULL", "sig", "sig+bkg"], help="make an animation of full / sig / separated sig+bkg data?")
...
upfile_name = os.path.join( args.input_directory, "ttbb_2l_dilep-bjet_delphes-met30-njet4p-btag2p",sample, sample+".root" )
...
if args.add_bkg: 
    ...
    upfile_name = os.path.join( args.input_directory, "ttbb_2l_dilep-bjet_delphes-met30-njet4p-btag2p",bkg, bkg+".root" )
    ...
    if ( args.LSTM ):
        ...
        with uproot.open( upfile_name ) as upfile:
            for name, branch in upfile["Events"].arrays( vector_branches, library = "np" ).items():
                ...
                vec_br_f[name] = branch
        # put columns side by side and transpose the innermost two axis
        v_bkg = np.column_stack( [np.stack( vec_br_f[name] ) for name in vector_branches] ).reshape( len(w_bkg), len(vector_branches), max_timestep ).transpose((0,2,1))

# the target vectors y (and y_bkg) now contain the SM and EFT-weights: y = (w_0, w_1, w_2) - y_bkg = (genWeight, 0, 0)
# normalizing to integrated luminosity W_0 gives new target weights:   w = (1, w_1/w_0, w_2/w_0)*W_0  -  w_bkg = (1,0,0)*W_0
...
# def eval_train ( var_evaluation, bkg = False ):
    # if not bkg: z = np.array( model(torch.Tensor(x), torch.Tensor(v)) )
    # else: z = np.array( model(torch.Tensor(x_bkg), torch.Tensor(v_bkg)) )
    # if not bkg: x_eval = np.array(xx[var_evaluation]) 
    # else: x_eval = np.array(xx_bkg[var_evaluation]) 
    # if reduce: x_eval = x_eval[0:red] if not bkg else x_eval[0:red_bkg]
    ...
    # for b in range ( 1,len(bins)-1 ):
        # for ind in range ( x_eval.shape[0] ):
            ...
            # if ( val > bins[b-1] and val<= bins[b] ):
                # if not bkg: 
                    # train_lin[b] += z[ind,0]* w[ind,0]  / w[ind,3]  
                    # train_quad[b]+= z[ind,1]* w[ind,0]  / w[ind,3]   
                # else: 
                    # train_lin[b] += z[ind,0] 
                    # train_quad[b]+= z[ind,1]                
    # name_var = var_evaluation if not bkg else var_evaluation+"_bkg"            
    ...
    if (args.animate == "FULL"):
        ...
        for b in range ( 1,len(bins)-1 ):
            for ind in range ( x_eval.shape[0] ):
                val = x_eval[ind]
                if ( val > bins[b-1] and val<= bins[b] ):
                    train_lin[b] += z[ind,0]* W_np[ind,0]  / W_np[ind,3]  
                    train_quad[b]+= z[ind,1]* W_np[ind,0]  / W_np[ind,3]   
                            
        name_var = var_evaluation          
        plots[name_var+'_lin' ].set_data( bins, train_lin[:,0]  )               
        plots[name_var+'_quad'].set_data( bins, train_quad[:,0] )
    
    if not (args.animate=="FULL"):
        ...
        if (args.animate=="sig+bkg"):
            ...
            for b in range ( 1,len(bins)-1 ):
                for ind in range ( x_eval_bkg.shape[0] ):
                    val = x_eval_bkg[ind]
                    if ( val > bins[b-1] and val<= bins[b] ):
                        train_lin[b] += z_bkg[ind,0]* w_bkg[ind,0]  / w_bkg[ind,3]  
                        train_quad[b]+= z_bkg[ind,1]* w_bkg[ind,0]  / w_bkg[ind,3]   
                                
            name_var = var_evaluation+'_bkg'          
            plots[name_var+'_lin' ].set_data( bins, train_lin[:,0]  )               
            plots[name_var+'_quad'].set_data( bins, train_quad[:,0] )  
      
# def eval_truth ( var_evaluation, bkg = False):
    # if not bkg: x_eval = np.array(xx[var_evaluation]) 
    # else: x_eval = np.array(xx_bkg[var_evaluation]) 
    # if reduce: x_eval = x_eval[0:red] if not bkg else x_eval[0:red_bkg]
    # hist, bins  = np.histogram( x_eval, bins=nbins, range=(config.plot_mva_variables[var_evaluation][0][0], config.plot_mva_variables[var_evaluation][0][1]) )
    ...
    # for b in range ( 1,len(bins)-1 ):
        # for ind in range ( x_eval.shape[0] ):
            ...
            # if ( val > bins[b-1] and val<= bins[b] ):
                # if not bkg: 
                    # truth_lin[b] +=w[ind,1]  
                    # truth_quad[b]+=w[ind,2]  
                # else: 
                    # truth_lin[b] +=w_bkg[ind,1]
                    # truth_quad[b]+=w_bkg[ind,2]
...
def eval_truth ( var_evaluation ):
    if (args.animate == "FULL"):
        ...
        for b in range ( 1,len(bins)-1 ):
            for ind in range ( x_eval.shape[0] ):
                val = x_eval[ind]
                if ( val > bins[b-1] and val<= bins[b] ):
                    truth_lin[b] +=W_np[ind,1]  
                    truth_quad[b]+=W_np[ind,2]  
    ...
    if not (args.animate == "FULL"):
        ...
        ax[index[i]].set_xlabel( config.plot_mva_variables[var_evaluation][1] )    
        #ax[index[i]].set_ylim((-0.0001,0.0001))
...
class LossLikelihoodFree(torch.nn.L1Loss):
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        for theta_base in self.base_points: #two base-points: 1,2
            ...
            loss += SM_ref*( theta_base*(target_lin/weight*SM_ref-input[:,0]) + theta_base**2*(target_quad/weight*SM_ref-input[:,1]) )**2
...
dir_name = str(args.sample)+'_mean_'+str(args.EFTCoefficients)+'_lr-'+str(args.lr)+'_lumi-'+str(int(lumi))+'_epx-'+str(n_epochs)+'_hs1-'+str(hidden_size)+'_hs2-'+str(hidden_size2)+'_hsc-'+str(hidden_size_comb)
if ( args.LSTM ): 
    dir_name = dir_name +  '_lstm-'+str(num_layers)+'_hs-lstm-'+str(hidden_size_lstm)
if (reduce): dir_name = dir_name + '_red-'+str(args.reduce)    
if (args.add_bkg): dir_name = dir_name + '+bkg'    
