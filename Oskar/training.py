...
argParser.add_argument("--plot_directory",     action="store",      default="genTops",                 help="plot sub-directory")
argParser.add_argument("--model",              action="store",      default="genTops",                 help="Which model?")
...
argParser.add_argument("--coefficients",       action="store",      default=['ctWRe'],       nargs="*", help="Which coefficients?")
...
for i_event in range(5):
    ...
    print ("  Features "+ "".join(["%s %3.2f "%( feature_name.replace('genJet_',''), training_features[i_event][i_feature]) for i_feature, feature_name in enumerate(model.feature_names)]) )

    print ("  w(ctWRe) = %+5.2e*(1%+5.2e*ctWRe%+5.2e*ctWRe^2)"%( training_weights[()][i_event], training_weights[('ctWRe',)][i_event]/training_weights[()][i_event], 0.5*training_weights[('ctWRe','ctWRe')][i_event]/training_weights[()][i_event] ) )
    print

# These are the plot options: model.plot_options

assert False, ""

# Below code that I commented.
