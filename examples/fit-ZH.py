...
model_t = [torch.nn.BatchNorm1d(n_features), torch.nn.ReLU(), torch.nn.Linear(n_features, hidden_layers[0])]
model_s = [torch.nn.BatchNorm1d(n_features), torch.nn.ReLU(), torch.nn.Linear(n_features, hidden_layers[0])]
for i_layer, layer in enumerate(hidden_layers):

    model_t.append(torch.nn.Linear(hidden_layers[i_layer], hidden_layers[i_layer+1] if i_layer+1<len(hidden_layers) else 1))
    if i_layer+1<len(hidden_layers):
        model_t.append( torch.nn.ReLU() )

    model_s.append(torch.nn.Linear(hidden_layers[i_layer], hidden_layers[i_layer+1] if i_layer+1<len(hidden_layers) else 1))
    if i_layer+1<len(hidden_layers):
        model_s.append( torch.nn.ReLU() )

model_t = torch.nn.Sequential(*model_t)
model_s = torch.nn.Sequential(*model_s)
print ("model_t\n", model_t)
print ("model_s\n", model_s)
    ...
    if (epoch % plot_every)==0:
        with torch.no_grad():
            ...
            for var in plot_vars:
                ...
                max_ = max( map( lambda h:h.GetMaximum(), [ h_truth_p, h_truth_pp, h_pred_p, h_pred_pp ] ))
