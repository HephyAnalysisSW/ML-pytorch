...
class NN:
    ...
    def predict_r_hat2(self, features, predicions, basepoints):
        ...
        linear_terms=torch.sum( torch.stack( [(1. + predictions[ci,:]*basepoints[c])**2 for ci,c in enumerate(self.coefficient_names) ]), dim=0)
        quadratic_terms= torch.sum( torch.stack( [torch.sum( torch.stack( [ predictions[(c_1,c_2)]*eft[c_2] for c_2 in coefficients[i_c_1:] ]), dim=0)**2 for i_c_1, c_1 in enumerate(self.coefficient_names) ] ), dim=0 )
